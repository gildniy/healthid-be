...
class Command(BaseCommand):
    args = "No Arguments"
    help = "A function to send a mail to the people"

    def _send_mail(self, payments_filename, products_filename, to_email):
        """
    ...
    def _write_csv(self, daily_sales, to_email, name_id):
        """
        ...
        self._send_mail(payments_filename, products_filename, to_email)
    ...
    def generate_stock_reports(self):
        """
            This function deletes old csv's present on the server
            and sends mails to the required people
        """
        self.logger(f"stock report program started at {datetime.datetime.now()}")
        ...
        self.logger("deleted old files present on the server.")
        ...
        self._write_csv(todays_total_sales, EMAIL_LIST , "managers")
        self.logger("sent mail to managers")
        ...
        for outlet_present in list(outlets_present):
            ...
            self._write_csv(outlets_sale_for_today, email_to, outlet_present)
            self.logger(f"sent mail to outlet {outlet_present}")
        self.logger("")
    
    def handle(self, *args, **options):
        self.generate_stock_reports()
